...
//                $ancho = isset($atRow->imgWidth) ? $uaRow->imgWidth : 200;
//                $alto = isset($atRow->imgHeight) ? $uaRow->imgHeight : 300;


				
				$insertSql .= '
					imgWidth	= '.$atRow->imgWidth.',
					imgHeight	= '.$atRow->imgHeight.',
			...
			if(!isset($returnData['err'])){
				if(mysql_query($insertSql)){
					...
					$tr .= '</td>';
					$tr .= '<td>';
					$tr .= '<button type="button" class="btn btn-sm btn-primary editBtn" data-id="'.$insertedID.'" data-a="getAdvertisement" data-u="'.$formPostUrl.'" data-w="$advertTypeWindow">';
		...
		if($uaRes && mysql_num_rows($uaRes) > 0){
			...
			//Code added by Hamlet
            $returnData['field']['aatIDe1'] = stripslashes($uaRow->atID);
			//End Code added by Hamlet
		...
		if(mysql_num_rows($uaRes) > 0){
            ...
            if($atIDe == ''){
                $returnData['err']['aatIDe'] = '<span class="bg-red tip">Please Enter Advertisement Type.</span>';
            }

            $atRes1 = mysql_query('SELECT * FROM advertisement_types WHERE atID = '.$atIDe.' AND isDeleted = "N"');

            if($atRes1 && mysql_num_rows($atRes1) > 0){
                $atRow = mysql_fetch_object($atRes1);

                $name = mysql_real_escape_string(stripslashes($atRow->atName));
                $ancho = $atRow->imgWidth;
			    $alto = $atRow->imgHeight;
