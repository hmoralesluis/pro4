<?
	session_name("aswakdubai");
	ini_set('session.cookie_domain', '.aswakdubai.com');
	session_set_cookie_params(0, '/', '.aswakdubai.com');
	session_start();
?>
<!DOCTYPE html>
<html>
				...
				if(m == 'true'){
					...
					var elf = $('#elfinder').elfinder({
						...
						getFileCallback : function(file) {
							<? if(isset($_REQUEST['p'])): ?>
							window.opener.receiveFile(file, getURLParameter('f'), getURLParameter('p')); //getURLParameter('f') in this f is Field
							<? else: ?>
