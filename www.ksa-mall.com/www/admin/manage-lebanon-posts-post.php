<?
	...
	$formPostUrl = 'manage-lebanon-posts-post.php';
	...
	if($action == 'addLbp'){
		...
		if($lbpTitle == ''){
			$returnData['err']['albpTitle'] = '<span class="bg-red tip">Please Enter Lebanon Title.</span>';
		}
		if($lbpImg == ''){
			$returnData['err']['albpImg'] = '<span class="bg-red tip">Please Select Lebanon Image.</span>';
		}
		if($lbpShortDesc == ''){
			$returnData['err']['albpShortDesc'] = '<span class="bg-red tip">Please Enter  Lebanon Short Description.</span>';
		}
		if($seoUrl == ''){
			$returnData['err']['aseoUrl'] = '<span class="bg-red tip">Please Enter  Lebanon SEO URL.</span>';
		}
		...
		if(!isset($returnData['err'])){
			...
			if(mysql_query($insertSql)){
				...
				$returnData['msg'] = '<span class="bg-green tip">Lebanon Post Added Successfully.</span>';
				...
				$tr .= '</button>'."\n";
				$tr .= '<button type="button" class="btn btn-sm btn-danger deleteIcon" data-placement="top" data-original-title="Delete" data-id="'.$insertedID.'" data-a="delLbp" data-u="'.$formPostUrl.'" data-at="Lebanon Post" data-numbering="lbpRow" data-column="1" >';
		...
		if(mysql_num_rows($lbpRes) > 0){
			...
			if($lbpTitle == ''){
				$returnData['err']['eBpTitle'] = '<span class="bg-red tip">Please Enter Lebanon Title.</span>';
			}
			if($lbpImg == ''){
				$returnData['err']['elbpImg'] = '<span class="bg-red tip">Please Select Lebanon Image.</span>';
			}
			if($lbpShortDesc == ''){
				$returnData['err']['elbpShortDesc'] = '<span class="bg-red tip">Please Enter  Lebanon Short Description.</span>';
			}
			if($seoUrl == ''){
				$returnData['err']['eseoUrl'] = '<span class="bg-red tip">Please Enter  Lebanon SEO URL.</span>';
			}
			...
			if(!isset($returnData['err'])){
				...
				if(mysql_query($updateSql)){
					$returnData['status'] = 0;
					$returnData['msg'] = '<span class="bg-green tip">Lebanon Post Updated Successfully.</span>';
					$btTag	= explode(',',$lbpTag);
					foreach($btTag as $key => $tag){
						$insertSql = mysql_query('
							INSERT INTO lebanon_tags SET
		...
		if(mysql_num_rows($lbpRes) > 0){
			...
			if(mysql_query($delSql)){
				$returnData['status'] = 0;
				$returnData['msg'] = '<span class="bg-green tip">Lebanon Post Deleted Successfully.</span>';
			}else {
				$returnData['msg'] = '<span class="bg-red tip">Error on Deleting Category, Please Contact Support.</span>';
			}
			
		}else { 
			$returnData['msg'] = '<span class="bg-red tip">Invalid Lebanon Post or Lebanon Post may Deleted, Please Contact Support.</span>';
		}
