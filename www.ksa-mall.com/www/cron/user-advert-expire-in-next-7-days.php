	...
	if($uRes && mysql_num_rows($uRes) > 0){
		while($uRow = mysql_fetch_object($uRes)){
			...
			if(mysql_num_rows($res) > 0){
				...
				$emailMsg .= '<td>Advertisement Name</td>';
				$emailMsg .= '</tr>';
				while($row = mysql_fetch_object($res)){
					if($expiryOn == ''){
						...
						$emailMsg .= '<tr>';
						$emailMsg .= '<td colspan="2">'.date('d-m-Y', strtotime($row->expiryOn)).'</td>';
						$emailMsg .= '</tr>';
					...
					$emailMsg .= '<td>'.stripslashes($row->uaName).' ['.stripslashes($row->atName).']</td>';
					$emailMsg .= '</tr>';
			...
			if($emailMsg != ''){
				...
				if($smtpHost != '' && $smtpPort != '' && $smtpUser != '' && $smtpPass != ''){
					...
					$mail->Subject	= 'Your Advertisement on '.$domain.' is about to expire';
