<?
	...
	#Delete Unfinished Add Business Listings
	mysql_query('DELETE FROM businesslistings WHERE userID = '.$userID.' AND subCatIDs IS NULL AND isNotifyEmailSent = "N" AND ');
	#End
	
	$expiryOn = date('Y-m-d', strtotime('+1 years'));
	...
	if(mysql_num_rows($listingRes) > 0){
		...
		$header = array(
			'siteTitle' => 'Add Business Listing', 
			'cMenu'		=> 'dashboard', 
			'breadcrumb'=> array(
				array('url' => 'my-business-listings.php', 'text' => 'My Business Listings'), 
				array('url' => '', 'text' => 'Add Business Listing')
			)
		);
	}else {
		$header = array(
