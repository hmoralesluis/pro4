...
if( $body.hasClass('map-fullscreen') ) {
    if( $(window).width() > 768 ) {
        ...
        $('.map-canvas:not(.no-fixed-height)').height( $(window).height() - $('.header').height() );
    }
    else {
...
function createHomepageGoogleMap(_latitude,_longitude,json){
    $.get("/new/assets/external/_infobox.js", function() {
        gMap();
    });
    function gMap(){
        ...
        for (var i = 0; i < json.data.length; i++) {
            ...
            var infoboxOptions = {
                ...
                closeBoxURL: "/new/assets/img/close.png",
        ...
        var clusterStyles = [
            {
                url: '/new/assets/img/cluster.png',
                height: 34,
                width: 34
            }
        ];
...
function multiChoice(sameLatitude, sameLongitude, json) {
    //if (clickedCluster.getMarkers().length > 1){
        ...
        $('.modal-window').load( '/new/assets/external/_modal-multichoice.html', function() {
