...
@section('page_title')
    <h3 class="header smaller lighter blue">Actualizar fabricante</h3>
@endsection
...
@section('content')
    ...
    @if (count($errors) > 0)
        <div class="alert alert-danger">
            <strong>Whoops!</strong> Hay problemas para actualizar el fabricante<br><br>
            <ul>
    ...
    @if(Session::has('flash_message'))
        ...
        <div class="alert alert-success">
            <strong>Bien!!!!</strong> Fabricante actualizado<br><br>
            <ul>
                <li>
    ...
    <form method="post" class="form-horizontal" action="{{url('admin/category/update/'.  $category->id)}}" enctype="multipart/form-data">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
        <div class="control-group">

            <div class="controls">
                <label for="form-field-1"> Nombre:  </label>
                <input type="text" name="name" id="form-field-1" placeholder="Nombre" required="true" value="{{$category->name}}"/>
            </div>
        </div>
